...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot per-class label counts across depth ranges."
    )
    parser.add_argument(
        "--tsv-path",
        default="/Users/fernandalecaros/Downloads/isiis_labels.tsv",
        help="TSV with 'Label' and 'depth' columns.",
    )
    parser.add_argument(
        "--bin-width",
        type=int,
        default=100,
        help="Depth bin width in meters (default: 100).",
    )
...
def main():
    ...
    df = bin_by_depth(df, depth_col='depth', bin_width=args.bin_width, range_col='Depth Range')
