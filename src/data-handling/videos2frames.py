...
def extract_frames(video_path, output_dir, frame_rate, verbose=False):
...
def process_video_file(params):
    video_path, base_output_dir, input_dir, frames_per_second, verbose = params
    ...
    os.makedirs(output_dir, exist_ok=True)
    extract_frames(video_path, output_dir, frames_per_second, verbose)

def process_videos(input_dir, output_dir, frames_per_second, verbose=False):
    video_files = []
    ...
    for root, dirs, files in tqdm(os.walk(input_dir), desc="Walking through directories"):
        for filename in files:
            if filename.endswith(".mp4"):
                ...
                video_files.append((video_path, output_dir, input_dir, frames_per_second, verbose))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--verbose', action='store_true', help='Print per-video processing details')
    ...
    process_videos(args.input, args.output, args.fps, args.verbose)
